...
CShaderAnimation2D::CShaderAnimation2D()
{}

CShaderAnimation2D::~CShaderAnimation2D()
{}

bool CShaderAnimation2D::Init(const std::string & PathName)
{
	//셰이더 로드하며 실패시 false를 반환한다.
	if (!LoadVertexShader("AnimationVS", TEXT("Animation2D.hlsl"), PathName))
	{
		return false;
	}

	if (!LoadPixelShader("AnimationPS", TEXT("Animation2D.hlsl"), PathName))
	{
		return false;
	}
